...
#     np.random.seed(seed)
#     rd.seed(seed)
#     os.environ['PYTHONHASHSEED'] = str(seed) #houjia
#     torch.manual_seed(seed) # cpu
#     torch.cuda.manual_seed_all(seed)  # gpu
#     torch.backends.cudnn.deterministic = True  #houjia
#     torch.backends.cudnn.benchmark = False #houjia
...
class Data(object):
    ...
    def create_adj_mat(self):
        ...
        # normalized_adj_single 是一个函数，用于对输入的矩阵进行归一化处理。归一化的目的是使矩阵的行和（或列和）为1，这通常用于图算法中，以表示节点之间的相对重要性或连接强度。
        # 在这个函数中，首先计算矩阵每行的和，然后取这些和的倒数（如果和为0，则倒数设置为0以避免除以0的错误）。接着，创建一个对角矩阵，其对角线上的元素就是这些倒数。最后，将这个对角矩阵与原始矩阵相乘，得到归一化后的矩阵。
